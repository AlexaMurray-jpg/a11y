...
[*Mini Knowledge Base*[^kb_1]]{.small}
...
*At the end is a glossary to help you identify the names of specialized types of charts, graphs, and diagrams you may find when remediating documents.*
...
: *Useful for corrupted PDFs, PDFs with many text errors: allows you to cleanly rerun OCR.*
...
For basic chart alt text guidance, one of the best sources of examples is [Section 508](https://www.section508.gov/create/alternative-text/#charts-graphs-and-diagrams).
...
- Endnote with structured information (lists, tables, paragraphs, etc).
  - If paragraphs of description are needed, you may wish to add both a *terse* and *verbose* version of descriptions, in a list.
...
<dt> Chemical Formulas/Equations </dt>
...
## Glossary: Diagrams, Graphs, Charts

AI can probably identify most types of graphs and charts you will see, but it can be helpful to have a reference on hand to look up the terms you need. This section will likely eventually be subsumed into *Content: What-To.*

Heat Map
  : \
    
    ```{r heatmap-example, echo=FALSE, out.width="40%"}
    data <- matrix(mtcars$mpg, mtcars$cyl, mtcars$disp)
    heatmap(data)
    ```

Box Plot/Box and Whisker Plot
  : \
    ```{r box-example, echo=FALSE, out.width="33%"}
    boxplot(mtcars$mpg)
    ```
...
Histogram
  : \
    ...
    ```{r hist-example, echo=FALSE, out.width="33%"}
    hist(mtcars$mpg)
    ```
  


[^kb_1]: Page always under construction. [Knowlege Base Definition | How to Use This Page](https://www.atlassian.com/itsm/knowledge-management/what-is-a-knowledge-base) [Example of Knowledge Base](https://kbx.macrium.com/macrium-reflect-x)
